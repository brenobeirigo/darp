...
output_filepath = os.path.join(
    *"darpsrp_parragh_2015/DARP/pr02_result.txt".split("/")
)
...
solution = solution_parser.parse_solution_from_filepath(
    os.path.join(root, output_filepath)
)
...
assert round(total_duration, 2) == round(
    solution.total_duration, 2
), f"{total_duration} != {solution.total_duration}"
assert round(total_transit, 2) == round(
    solution.total_transit, 2
), f"{total_transit} != {solution.total_transit}"
...
for v_sol in solution.vehicle_routes:
    ...
    for n in v_sol.visits:
        ...
        if type(node) == DropoffNode:
            ...
            print(
                ...
                o.pos,
                node.pos,
                f"ride: {round(ride_delay,2)}",
                round(shortest_dist, 2),
                f"total transit: {round(transit,2)}",
                f"      total ride: {round(total_ride,2)}",
                f"t: {round(t,2)}",
            )
    ...
    for o, d in zip(v_sol.visits[:-1], v_sol.visits[1:]):
        ...
        cost += dist
        print(
            f"arrival({o.id:>6}) = {arrival:8.3f} / {o.b:7.3f} (w:{vehicle_waiting_node:>8.3f}) \t\t departure({o.id:>6}) = {departure:8.3f} \t\t dist({o.id:>6},{d.id:>6}) = {dist:7.3f} / ({total_distance:7.3f}) + d[{o.id:>6}] = {n_o.service_duration:7.3f} \t\t ({n_o.tw.earliest:7},{n_d.tw.latest:7}) \t\t node w: {node_waiting:7.3f} \t\t cost: {cost:7.3f}"
        )
    ...
    print(
        f"arrival({d.id:>6}) = {arrival:8.3f} / {d.b:7.3f} (w:{vehicle_waiting_node:>8.3f}) \t\t departure({d.id:>6}) = {departure:8.3f} \t\t dist({d.id:>6},{0:>6}) = {0:7.3f} / ({total_distance:7.3f}) + d[{d.id:>6}] = {n_d.service_duration:7.3f} \t\t ({0:7},{n_d.tw.latest:7}) \t\t node w: {node_waiting:7.3f} \t\t cost: {cost:7.3f}"
    )
...
print(
    "total waiting time:",
    vehicle_waiting,
    vehicle_waiting / (len(v_sol.visits) - 2),
)
print(solution)
