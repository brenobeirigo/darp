...
- **$t^k$**: Continuous variable representing the total driving time (i.e., $B^k_2 - B^k_2$) of vehicle $k$.
...
Below is a detailed table with the values for each of the parameters mentioned in the question:
...
| $s$                   | Average speed vehicles  (km/h).                       | 25km/h, 50km/h                 |
...
### Step 2: Visualizing instances (sanity check)
...
To facilitate testing, the scenario settings proposed in the assignment are encoded in a `json` (see below) in folder `{python} folder_path`.
...
### Step 4: Run scenarios and save results
