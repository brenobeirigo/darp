...
class Route:
    ...
    def is_feasible(self, distance_matrix):
        ...
        earliest_departure_from_current_node = (
            current_node.arrival + current_node.service_delay
        )
        ...
        for next_node in self.nodes[1:]:
            ...
            earliest_arrival_at_next_node = (
                earliest_departure_from_current_node + dist
            )
            arrival_next_node = max(
                earliest_arrival_at_next_node, next_node.tw.earliest
            )
            departure_next_node = arrival_next_node + next_node.service_delay

            departure_current_node = max(
                latest_departure_from_current_node,
                earliest_departure_from_current_node,
            )
